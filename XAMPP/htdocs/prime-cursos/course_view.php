...
// Fallbacks: si no se encuentra, intentar buscar por índice numérico (1-based), por "slug" o por título normalizado
if(!$course){
  // 1) índice numérico (cliente puede enviar id=1..n)
  if(is_numeric($id)){
    $idx = intval($id) - 1;
    if(isset($courses[$idx])){ $course = $courses[$idx]; }
  }
}

if(!$course){
  // 2) buscar por campo 'slug'
  foreach($courses as $c){ if(isset($c['slug']) && (string)$c['slug'] === (string)$id){ $course = $c; break; } }
}

if(!$course){
  // 3) intentar normalizar título a "slug-like" y compararlo
  $norm = function($s){ $s = mb_strtolower(trim($s)); $s = preg_replace('/[^a-z0-9\-]+/u','-', $s); $s = preg_replace('/\-+/','-',$s); $s = trim($s,'-'); return $s; };
  $idnorm = $norm((string)$id);
  if($idnorm !== ''){
    foreach($courses as $c){
      if(!empty($c['title']) && $norm($c['title']) === $idnorm){ $course = $c; break; }
    }
  }
}
...
<div class="container">
  ...
  <?php if(!$hasAccess): ?>
    ...
    <div class="payment-actions">
      <div class="btn-row">
        <button id="paypalBuy" class="brand-btn brand-paypal" title="Pagar con PayPal">
          <img src="/prime-cursos/assets/img/paypal.svg" alt="PayPal">
        ...
        <form id="hotmartForm" style="display:inline-block">
          ...
          <button type="submit" class="brand-btn brand-hotmart" title="Comprar en Hotmart">
            <img src="/prime-cursos/assets/img/hotmart.svg" alt="Hotmart">
            <span>Comprar en Hotmart</span>
          </button>
        </form>
        <button id="mpBuy" class="brand-btn brand-mp" title="Pagar con Mercado Pago">
          <img src="/prime-cursos/assets/img/mercadopago.svg" alt="Mercado Pago">
          <span>Pagar con Mercado Pago</span>
        </button>
        <button id="addToCartBtn" class="brand-btn brand-cart small" title="Agregar al carrito">
          <img src="/prime-cursos/assets/img/cart.svg" alt="Agregar al carrito">
