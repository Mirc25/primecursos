...
window.addEventListener('resize', ()=>{ updateSlider(); });});  renderSlider();// ensure images have loaded before calculating slider positions
nwindow.addEventListener('load', ()=>{// Slider init (uses courses from data.js)}  });    fetch('mp_create_pref.php', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ items: mpItems, back_urls: back }) }).then(function(r){ return r.json(); }).then(function(j){ if(!j || !j.ok){ alert('Error creando preferencia Mercado Pago'); console.error(j); return; } if(j.redirectUrl){ window.location = j.redirectUrl; } else alert('No se obtuvo URL de redirección de Mercado Pago'); }).catch(function(err){ console.error(err); alert('Error contactando al servidor Mercado Pago'); });    var back = { success: window.location.href, failure: window.location.href };    var mpItems = cart.map(function(i){ var p = Number(i.price || 0); var d = Number(i.discount_pct || 0); var unit = (d > 0) ? (p * (1 - (d/100))) : p; return { title: i.title || 'Producto', quantity: i.qty || 1, unit_price: Number(unit.toFixed(2)), currency_id: 'ARS' }; });    if(cart.length===0){ alert('El carrito está vacío.'); return; }  $('mpCheckoutBtn').addEventListener('click', function(){
n    // Reuse same logic as the mercadopago branch in checkout()if($('mpCheckoutBtn')){n// Direct Mercado Pago button (bypass prompt)